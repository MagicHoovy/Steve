...
public class TransactionRepositoryImpl implements TransactionRepository {
    ...
    public TransactionDetails getDetails(int transactionPk) {
        ...
        String chargingTime = "";
        DateTime startTime = transaction.value5();
        DateTime stopTime = transaction.value7();
        
        if (stopTime != null) {
            long durationInMinutes = (stopTime.getMillis() - startTime.getMillis()) / (1000 * 60);
            long hours = durationInMinutes / 60;
            long minutes = durationInMinutes % 60;
            chargingTime = String.format("%dh %dm", hours, minutes);
        } else {
            // For active transactions, calculate time until now
            long durationInMinutes = (DateTime.now().getMillis() - startTime.getMillis()) / (1000 * 60);
            long hours = durationInMinutes / 60;
            long minutes = durationInMinutes % 60;
            chargingTime = String.format("%dh %dm (Active)", hours, minutes);
        }

        // Get connector status
        String status = ctx.select(CONNECTOR_STATUS.STATUS)
                         .from(CONNECTOR_STATUS)
                         .where(CONNECTOR_STATUS.CONNECTOR_PK.eq(
                             ctx.select(CONNECTOR.CONNECTOR_PK)
                                .from(CONNECTOR)
                                .where(CONNECTOR.CHARGE_BOX_ID.eq(transaction.value2()))
                                .and(CONNECTOR.CONNECTOR_ID.eq(transaction.value3()))
                         ))
                         .orderBy(CONNECTOR_STATUS.STATUS_TIMESTAMP.desc())
                         .limit(1)
                         .fetchOneInto(String.class);

        // Map the transaction record to Transaction object
        Transaction mappedTransaction = Transaction.builder()
                                                .id(transaction.value1())
                                                .chargeBoxId(transaction.value2())
                                                .connectorId(transaction.value3())
                                                .ocppIdTag(transaction.value4())
                                                .startTimestamp(startTime)
                                                .startValue(transaction.value6())
                                                .stopTimestamp(stopTime)
                                                .stopValue(transaction.value8())
                                                .stopReason(transaction.value9())
                                                .stopEventActor(transaction.value12())
                                                .build();

        return TransactionDetails.builder()
                               .transaction(mappedTransaction)
                               .values(values)
                               .nextTransactionStart(nextTx)
